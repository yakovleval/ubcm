...
RegisterResult<RegisterAddress> resolve_selector(const RegisterSelector& s,const ResolutionContext& x){const NameResolver* r=nullptr;switch(s.class_id){case RegisterClass::procedure:r=x.procedure;break;case RegisterClass::superlocal:r=x.superlocal;break;case RegisterClass::local:r=x.local;break;case RegisterClass::global:r=x.global;break;}if(!r)return std::unexpected(RegisterError{RegisterError::Code::not_found,"resolver is unavailable"});return r->resolve_address(s.name);}
}
